/*
  ...
  Shows Coaching Cues, Setup, Standards, and Quick Copy — without leaving the drill library.
  Uses Sheet (side panel) on desktop, Drawer (bottom sheet) on mobile.
*/
...
export function DrillQuickPreview({ drillId, open, onOpenChange }: DrillQuickPreviewProps) {
  ...
  const content = (
    <div className="flex flex-col h-full">
      {/* Header info */}
      <div className="flex-shrink-0 pb-3 border-b border-t1-border mb-4">
        <div className="flex items-center justify-between gap-2 mb-2">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 rounded-md border text-[10px] font-semibold uppercase tracking-wider transition-all flex-shrink-0 ${
              favorited
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pb-4 -mx-1 px-1">
        ...
        <div className="bg-t1-blue/5 border border-t1-blue/20 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-display text-xs font-semibold uppercase tracking-wider text-t1-blue">
              Coaching Cues
            </h3>
            <button
              onClick={handleCopy}
        ...
        {/* Setup */}
        <div className="bg-t1-surface border border-t1-border rounded-lg p-4">
          <h3 className="font-display text-xs font-semibold uppercase tracking-wider text-t1-text mb-2">
            Setup
          </h3>
        ...
        {/* Standards */}
        <div className="bg-t1-surface border border-t1-border rounded-lg p-4">
          <h3 className="font-display text-xs font-semibold uppercase tracking-wider text-t1-text mb-2 flex items-center gap-1.5">
          ...
          <ul className="space-y-1.5">
            {drill.standards.map((s, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-t1-text/80">
                <span className="text-t1-blue mt-0.5">&#10003;</span> {s}
              </li>
            ))}
          </ul>
        </div>
        ...
        {/* Common Breakdowns */}
        <div className="bg-t1-red/5 border border-t1-red/15 rounded-lg p-4">
          <h3 className="font-display text-xs font-semibold uppercase tracking-wider text-t1-red mb-2">
            Common Breakdowns
          </h3>
          <ul className="space-y-1.5">
            {drill.commonBreakdowns.map((b, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-t1-text/80">
                <span className="text-t1-red mt-0.5">&times;</span> {b}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="flex-shrink-0 pt-3 border-t border-t1-border mt-auto">
        <Link
          ...
          className="flex items-center justify-center gap-2 w-full py-2.5 bg-t1-blue/10 text-t1-blue text-xs font-semibold uppercase tracking-wider rounded-lg hover:bg-t1-blue/20 transition-colors no-underline"
  ...
  if (isMobile) {
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="bg-t1-bg border-t1-border max-h-[85vh]">
          <DrawerHeader className="pb-0">
            <div className="flex items-center justify-between">
              <DrawerTitle className="font-display text-base font-bold uppercase tracking-wide text-t1-text">
                {drill.name}
              </DrawerTitle>
              <DrawerClose className="text-t1-muted hover:text-t1-text">
                <X className="w-5 h-5" />
              </DrawerClose>
            </div>
          </DrawerHeader>
