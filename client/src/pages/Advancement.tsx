/*
  ADVANCEMENT PAGE: Progression decisions, approval flow, statuses
  Clear, coach-led framework for player movement
*/
...
const statusIcons: Record<string, React.ReactNode> = {
  ...
  Deferred: <XCircle className="w-5 h-5 text-red-500" />,
};

const statusColors: Record<string, string> = {
  Approved: 'border-green-200 bg-green-50',
  Pending: 'border-amber-200 bg-amber-50',
  Deferred: 'border-red-200 bg-red-50',
};
    ...
    <div>
      ...
      <div className="container mt-8 space-y-8">
        ...
        <section className="bg-white border border-border rounded-lg p-6">
          <h2 className="font-display text-lg font-semibold uppercase tracking-wide text-t1-charcoal mb-4 flex items-center gap-2">
            <Shield className="w-5 h-5 text-t1-green" /> Advancement Philosophy
          </h2>
            ...
            {advancementInfo.statuses.map((status) => (
              <div key={status.name} className={`border rounded-lg p-5 ${statusColors[status.name]}`}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-display text-base font-semibold uppercase tracking-wide text-t1-charcoal">
                    {status.name}
                  </h3>
                </div>
                <p className="text-sm text-foreground/80 leading-relaxed">{status.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-white border border-border rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {advancementInfo.factors.map((factor, i) => (
              <div key={i} className="flex items-start gap-3 bg-secondary/30 rounded-md p-3">
        ...
        <section className="bg-white border border-border rounded-lg p-6">
          ...
          <div className="space-y-3">
            {advancementInfo.approvalChain.map((item, i) => (
              <div key={i} className="flex items-center gap-4 bg-secondary/30 rounded-md p-4">
                <div className="flex items-center gap-3 flex-1">
                  <span className="w-6 h-6 rounded-full bg-t1-green text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0">
                    {i + 1}
                  </span>
                  <span className="text-sm font-medium text-t1-charcoal">{item.stage}</span>
                </div>
                <ArrowRight className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                <span className="text-sm font-semibold text-t1-green bg-t1-green/10 px-3 py-1 rounded">
                  {item.owner}
                </span>
              </div>
            ))}
        ...
        <section className="bg-white border border-border rounded-lg p-6">
          <h2 className="font-display text-lg font-semibold uppercase tracking-wide text-t1-charcoal mb-4 flex items-center gap-2">
            <MessageSquare className="w-5 h-5 text-t1-green" /> Family Communication Standard
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
