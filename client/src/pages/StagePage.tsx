/*
  STAGE PAGE: Tier 1 Performance — Cold Dark Brand
  Individual class/level detail page
*/
...
export default function StagePage() {
  ...
  if (!stage) {
    return (
      <div className="container py-16 text-center">
        <h1 className="font-display text-2xl font-bold uppercase text-t1-text">Stage Not Found</h1>
        <Link href="/pathway" className="text-t1-blue mt-4 inline-block">Back to Pathway</Link>
      </div>
    );
  }
  ...
  const accent = stageAccent[stage.id] || 'border-t1-blue';
  const bg = stageBg[stage.id] || 'bg-t1-blue';
    ...
    <div>
      ...
      <section className={`border-b-4 ${accent} bg-t1-surface`}>
        <div className="container py-6">
          <Link href="/pathway" className="inline-flex items-center gap-1.5 text-sm text-t1-muted hover:text-t1-blue no-underline mb-4">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-t1-muted uppercase tracking-wider mt-1">{stage.subtitle}</p>
              {stage.contentStatus !== 'complete' && (
                <div className="mt-3 inline-flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 px-3 py-1.5 rounded text-xs font-medium">
      ...
      <div className="container mt-8 space-y-8">
        ...
        <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
          <h2 className="font-display text-lg font-bold uppercase tracking-wide text-t1-text mb-3">
            Purpose
          </h2>
          <p className="text-sm text-t1-text/80 leading-relaxed">{stage.purpose}</p>
        </section>

        {/* Two Column: Priorities + Non-Negotiables */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
            <h2 className="font-display text-lg font-bold uppercase tracking-wide text-t1-text mb-4">
              Coaching Priorities
            </h2>
            ...
            <ul className="space-y-2.5">
              {stage.nonNegotiables.map((n, i) => (
                <li key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-t1-text/80">{n}</span>
                </li>
              ))}
            </ul>
          </section>
        ...
        <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {stage.commonMistakes.map((m, i) => (
              <div key={i} className="flex items-start gap-3 bg-t1-red/5 border border-t1-red/10 rounded-md p-3">
                <XCircle className="w-4 h-4 text-t1-red flex-shrink-0 mt-0.5" />
                <span className="text-sm text-t1-text/80">{m}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        {stageAssessments.length > 0 && (
          <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-lg font-bold uppercase tracking-wide text-t1-text">
                Assessment Benchmarks
              </h2>
              <Link href="/assessments" className="text-sm text-t1-blue font-medium hover:underline no-underline flex items-center gap-1">
            ...
            {stageAssessments.map((assessment) => (
              <div key={assessment.stageName} className="mb-6 last:mb-0">
                <h3 className="font-display text-sm font-bold uppercase tracking-wider text-t1-muted mb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {assessment.categories.map((cat) => (
                    <div key={cat.name} className="bg-secondary/50 rounded-md p-3">
                      <h4 className="text-xs font-semibold uppercase tracking-wider text-t1-text mb-2 flex items-center gap-1.5">
                        <Target className="w-3 h-3 text-t1-blue" />
                        {cat.name}
                      </h4>
                      <ul className="space-y-1">
                        {cat.standards.map((s, i) => (
                          <li key={i} className="text-xs text-t1-muted">&bull; {s}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
        ...
        <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
          ...
          <p className="text-sm text-t1-text/80 leading-relaxed mb-3">{stage.advancementExpectations}</p>
          <div className="flex items-center gap-2 text-xs text-t1-muted">
            <span>Approval Owner:</span>
            <span className="font-semibold text-t1-blue bg-t1-blue/10 px-2 py-0.5 rounded">{stage.advancementOwner}</span>
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/drills?level=${stage.id}`} className="text-sm text-t1-blue font-medium hover:underline no-underline flex items-center gap-1">
          ...
          {stageDrills.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {stageDrills.slice(0, 6).map((drill) => (
                ...
                >
                  ...
                  <p className="text-xs text-t1-muted mt-1 line-clamp-2">{drill.objective}</p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[10px] bg-t1-blue/10 text-t1-blue px-2 py-0.5 rounded font-medium uppercase tracking-wider">
            ...
            <div className="bg-t1-navy/30 border border-t1-border rounded-lg p-6 text-center">
