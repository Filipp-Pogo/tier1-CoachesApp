/*
  ASSESSMENTS: Stage-by-stage competency standards
  Searchable by level, clean reference format
*/
  ...
  return (
    <div>
      ...
      <section className="relative h-40 sm:h-52 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-t1-charcoal/90 via-t1-charcoal/80 to-t1-charcoal/60" />
      ...
      <div className="container mt-6 space-y-6">
        ...
        <div className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
          <Target className="w-3.5 h-3.5 text-t1-green" />
        ...
        {filteredAssessments.map((assessment, idx) => (
          <section key={idx} className="bg-white border border-border rounded-lg overflow-hidden">
            <div className="bg-t1-green/5 border-b border-border px-6 py-4">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {assessment.categories.map((cat) => (
                  <div key={cat.name} className="bg-secondary/30 rounded-lg p-4">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-t1-charcoal mb-3 flex items-center gap-1.5 border-b border-border pb-2">
                    ...
                    <ul className="space-y-2">
                      {cat.standards.map((standard, i) => (
                        <li key={i} className="flex items-start gap-2">
                          <CheckCircle2 className="w-3 h-3 text-t1-green flex-shrink-0 mt-0.5" />
        ...
        {(activeStage === 'all' || stagesWithoutAssessments.includes(activeStage)) && (
          <div className="bg-t1-sand-light/50 border border-t1-sand rounded-lg p-6">
            <div className="flex items-start gap-3">
              <AlertTriangle className="w-5 h-5 text-t1-sand flex-shrink-0 mt-0.5" />
              <div>
                <h3 className="font-display text-sm font-semibold uppercase tracking-wide text-t1-charcoal mb-1">
