/*
  ...
  Coaching expectations and professional standards
*/
  ...
  return (
    <div>
      ...
      <div className="container mt-8 space-y-6">
        {/* Intro Banner */}
        <div className="bg-t1-navy rounded-lg p-6 sm:p-8 border border-t1-border">
          <div className="flex items-start gap-4">
            <Shield className="w-8 h-8 text-t1-blue flex-shrink-0" />
            <div>
        ...
        {coachStandards.map((section, idx) => {
          ...
          return (
            <section key={idx} className={`bg-t1-surface border rounded-lg p-6 ${isNegative ? 'border-t1-red/30' : 'border-t1-border'}`}>
              <h2 className={`font-display text-lg font-semibold uppercase tracking-wide mb-4 ${isNegative ? 'text-t1-red' : 'text-t1-text'}`}>
                {section.category}
              </h2>
              <ul className="space-y-3">
                {section.items.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    {isNegative ? (
                      <XCircle className="w-4 h-4 text-t1-red/60 flex-shrink-0 mt-0.5" />
                    ) : (
                      <CheckCircle2 className="w-4 h-4 text-t1-blue flex-shrink-0 mt-0.5" />
                    )}
                    <span className="text-sm text-t1-text/80">{item}</span>
                  </li>
                ))}
              </ul>
            </section>
        ...
        <section className="bg-t1-surface border border-t1-border rounded-lg p-6">
          <h2 className="font-display text-lg font-semibold uppercase tracking-wide text-t1-text mb-3">
            Remember
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="flex items-start gap-2">
                ...
                <span className="text-sm text-t1-text/80">{v}</span>
              </div>
            ))}
          </div>
        </section>
